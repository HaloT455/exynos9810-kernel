...
void susfs_set_uname(void __user **user_info) {
	...
	if (!memchr(info.release, '\0', sizeof(info.release)) ||
	    !memchr(info.version, '\0', sizeof(info.version))) {
		info.err = -EINVAL;
		goto out_copy_to_user;
	}
...
static DEFINE_MUTEX(susfs_fake_cmdline_or_bootconfig_mutex);
...
void susfs_set_cmdline_or_bootconfig(void __user **user_info) {
	...
	if (*info->fake_cmdline_or_bootconfig == '\0' ||
	    !memchr(info->fake_cmdline_or_bootconfig, '\0', sizeof(info->fake_cmdline_or_bootconfig))) {
		info->err = -EINVAL;
		goto out_copy_to_user;
	}

	mutex_lock(&susfs_fake_cmdline_or_bootconfig_mutex);
	strscpy(fake_cmdline_or_bootconfig,
			info->fake_cmdline_or_bootconfig,
			SUSFS_FAKE_CMDLINE_OR_BOOTCONFIG_SIZE - 1);
	mutex_unlock(&susfs_fake_cmdline_or_bootconfig_mutex);
...
void susfs_spoof_cmdline_or_bootconfig(struct seq_file *m) {
	/* seq_puts is not retryable: retries would append duplicate command lines. */
	mutex_lock(&susfs_fake_cmdline_or_bootconfig_mutex);
...
DEFINE_STATIC_KEY_FALSE(susfs_is_open_redirect_set);
...
void susfs_add_open_redirect(void __user **user_info) {
	...
	if (!memchr(info.target_pathname, '\0', sizeof(info.target_pathname)) ||
	    !memchr(info.redirected_pathname, '\0', sizeof(info.redirected_pathname))) {
		info.err = -EINVAL;
		goto out_copy_to_user;
	}
...
out_path_put_target_path:
	if (!info.err)
		static_branch_enable(&susfs_is_open_redirect_set);
...
struct filename *susfs_open_redirect_spoof_do_sys_openat(struct inode *inode) {
	...
	hash_for_each_possible_rcu(OPEN_REDIRECT_HLIST, entry, node, inode->i_ino) {
		if (entry->target_ino == inode->i_ino && !entry->reversed_lookup_only &&
...
int susfs_open_redirect_spoof_vfs_readlink(struct inode *inode, char __user *buffer, int buflen) {
	...
	hash_for_each_possible_rcu(OPEN_REDIRECT_HLIST, entry, node, inode->i_ino) {
			...
			entry->target_dev == inode->i_sb->s_dev)
		{
			...
			len = min_t(size_t, strlen(entry->info.redirected_pathname), buflen);
...
/* From KernelSU */
	...
	*out = m;
	/* The group's attached-mark reference owns the mark from here. */
	fsnotify_put_mark(m);
	return 0;
}
...
{
	struct cred *cred = ksu_cred ? prepare_kernel_cred(NULL) : NULL;
	int ret = 0;
	...
	/* Follow the restored KSU domain; donor hard-codes a different one. */
