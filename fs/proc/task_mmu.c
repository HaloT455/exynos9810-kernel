...
#ifdef CONFIG_KSU_SUSFS_SUS_MAP
		/* Keep pagemap offsets stable; masked pages must not shift entries. */
		for (vma = find_vma(mm, start_vaddr); vma && vma->vm_start < end;
		     vma = vma->vm_next) {
			unsigned long first, last;
			if (!vma->vm_file || !SUSFS_IS_INODE_SUS_MAP(file_inode(vma->vm_file)))
				continue;
			first = (max(start_vaddr, vma->vm_start) - start_vaddr) >> PAGE_SHIFT;
			last = (min(end, vma->vm_end) - start_vaddr) >> PAGE_SHIFT;
			last = min_t(unsigned long, last, pm.pos);
			if (first < last)
				memset(pm.buffer + first, 0, (last - first) * PM_ENTRY_BYTES);
		}
#endif
		up_read(&mm->mmap_sem);
		start_vaddr = end;
