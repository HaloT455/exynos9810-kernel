...
extern struct static_key_false susfs_is_open_redirect_set;
		...
		const struct open_flags *op)
{
	...
	struct file *filp;
#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
	struct filename *redirected = NULL;
	struct path redirect_path;

	/* Resolve before opening: never run do_last() twice on one struct file. */
	if (static_branch_unlikely(&susfs_is_open_redirect_set) &&
	    !filename_lookup(dfd, getname_kernel(pathname->name),
			     flags & (LOOKUP_FOLLOW | LOOKUP_DIRECTORY),
			     &redirect_path, NULL)) {
		if (SUSFS_IS_INODE_OPEN_REDIRECT_WITHOUT_UID_CHECK(
				redirect_path.dentry->d_inode))
			redirected = susfs_open_redirect_spoof_do_sys_openat(
					redirect_path.dentry->d_inode);
		path_put(&redirect_path);
		if (IS_ERR(redirected))
			return ERR_CAST(redirected);
		if (redirected)
			pathname = redirected;
	}
#endif
