...
# Restore the original KernelSU-Next module stack without SUSFS.
# Keep the 25 August CPU driver; this profile only changes KSU integration.
BUILD_EROFS_ENFORCING_LEGACY_KSU_RELEASE(){
CR_DATE=${DS_ACK_BUILD_DATE:-08.30.2026}
CR_COMPILER=${DS_ACK_COMPILER:-4}
CR_MKZIP=y
CR_CLEAN=${DS_ACK_CLEAN:-n}
CR_KSU=y
CR_SELINUX=2
CR_ZIP_SUFFIX=
CR_ZIP_NAME=$CR_NAME-$CR_VERSION-$CR_DATE-Enforcing-KernelSU-Restored-A55-2GHz-OneUI7-erofs-dtb

if [ "$(git -C "$CR_DIR/KernelSU-Next" rev-parse HEAD)" != "1ce19e536de730829b62d0d23dbd96c6ec809e2f" ]; then
    echo "ERROR: this profile requires the original 05 August KernelSU commit."
    return 1
fi
if [ ! -x "${DS_ACK_TOOLCHAIN:-$CR_TC/clang-20.0.0-r547379}/bin/clang" ]; then
    echo "ERROR: Google Clang 20 r547379 is missing."
    return 1
fi
BUILD_ALL || return 1
}
...
# Main Menu
if [ "$1" = "--erofs-enforcing-legacy-ksu-release" ]; then
BUILD_EROFS_ENFORCING_LEGACY_KSU_RELEASE
exit $?
fi
