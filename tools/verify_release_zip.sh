        ...
        case "$(basename "$ZIP_PATH")" in
            *KernelSU-Restored*)
                ...
                if grep -q '^CONFIG_KSU_SUSFS.*=y$' "$config"; then
                    echo "ERROR: restored KernelSU release still enables SUSFS" >&2
                    exit 1
                fi
                for option in EROFS_FS_XATTR EROFS_FS_POSIX_ACL EROFS_FS_SECURITY SECURITY_SELINUX; do
